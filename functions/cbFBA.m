function lb_ub = cbFBA(IM,err)
% calculates the flux ranges with cbFBA method
%
% USAGE:
%     lb_ub = cbFBA(IM, err)
%
...
%
% OUTPUT:
%     lb_ub = net flux range for reactions after applying cbFBA  
...
fk = [zeros(r,1);ones(c,1)];%factors for objective function
...
if size(minFlux,1) > 0
    minFluxSum = sum(minFlux(r+1:end,1));%fk = sum of cmpx fluxes
